...
<head>
	...
	<meta charset="UTF-8">
	<link rel="stylesheet" href="/res/css/reset.css"><link rel="stylesheet" href="/res/css/common.css"><link rel="stylesheet" href="/res/css/scanlines.css"><script src="/res/js/jquery.js"></script><script src="/res/js/howler.js"></script><script src="/res/js/cookies.js"></script>
	<script>
		document.addEventListener("DOMContentLoaded", function(event) { 
			var noise = new Howl({
				src: ['/res/audio/bgm/menu.ogg'],
				loop: true,
				volume: 0.5
			});
			...
			if (getCookie('prefs') !== '') {
				var prefs = JSON.parse(getCookie("prefs"));

				if (prefs.scanlines === false) {
					$('fieldset input[type="radio"][name="scanlines"][value="off"]').prop("checked", true);
				}

				if (prefs.status !== undefined) {
					$(`fieldset input[type="radio"][name="status"][value="${prefs.status}"]`).prop("checked", true);
				}

				if (prefs.home !== undefined) {
					if (prefs.home.bgm !== undefined) {
						$(`fieldset input[type="radio"][name="homebgm"][value="${prefs.home.bgm}"]`).prop("checked", true);
					}

					if (prefs.home.intro !== undefined) {
						if (prefs.home.intro.intro !== undefined) {
							$(`fieldset input[type="radio"][name="homeintro"][value="${prefs.home.intro.intro}"]`).prop("checked", true);
						}

						if (prefs.home.intro.always === true) {
							$('fieldset input[name="introalways"]').prop("checked", true);
						}
					}
				}
			} else {
			...
			$('fieldset input').click(function() {
				...
				switch (this.name) {
					...
					case 'homeintro':
						...
						prefs.home.intro.intro = this.value;
						break;
					...
					case 'introalways':
						...
						if ($('input[name="introalways"]').is(':checked')) {
							prefs.home.intro.always = true;
						} else {
							prefs.home.intro.always = false;
						}

						break;

					case 'status':
						prefs.status = this.value;
				}

				setCookie("prefs", JSON.stringify(prefs), 3000);
			});
			...
			$(document).keydown(function(e) {
				...
				if (kkeys.toString().indexOf(konami) >= 0) {
					...
					$("#set").removeClass("nope");
					$("#show").removeClass("nope");
					$("#clear").removeClass("nope");
				}
			});
		});
	</script>
...
<div class="seperator"></div><div class="padded seethru">
	...
	<fieldset>
		<legend><h2 style="margin: 0;">Scanlines</h2></legend>
		<p>Enable or disable the static overlay.</p>
		<input name="scanlines" type="radio" value="on" checked>Enabled</input>
		<input name="scanlines" type="radio" value="off">Disabled</input>
	</fieldset>
	<fieldset>
		<legend><h2 style="margin: 0;">Home</h2></legend>
		<p>BGM</p>
		<input name="homebgm" type="radio" value="new" checked>New</input>
		<input name="homebgm" type="radio" value="old">Old</input>
		<input name="homebgm" type="radio" value="tor">Alt</input>
		<p>Intro</p>
		<input name="homeintro" type="radio" value="new" checked>New</input>
		<input name="homeintro" type="radio" value="old">Old</input>
		<br><br>
		<input type="checkbox" name="introalways">Always Play Intro</input>
	</fieldset>
	<fieldset>
		<legend><h2 style="margin: 0;">Status</h2></legend>
		<p>Toggle between the new and old status page.</p>
		<input name="status" type="radio" value="new" checked>New</input>
		<input name="status" type="radio" value="old">Old</input>
	</fieldset>
	<button id="set" class="normal nope">Set</button>
	<button id="show" class="normal nope">Show</button>
	<button id="clear" class="normal nope">Clear</button>
	<br><br><br>
</div></body>
</html>
