	...
	<script>
		document.addEventListener("DOMContentLoaded", function(event) { 
			setInterval(function(){
				...
				var slots_left = $("#slots").val() - 3000;
				$("#slot").text(slots_left);
				
				var extra_days = (28000 - ($("#slots").val() - 3000)) / 50;
				$("#up").text(extra_days);
				...
				var dat = new Date(1498921200000);

				$("#unlinkdate").text(dat.addDays(extra_days).toLocaleDateString());
				$("#slotsss").text($("#slots").val());
...
<div class="seperator"></div><div class="padded seethru">
			<br><br>
	<p>We got extra Aurora, which we can either spend on days to extend the lifespan of Life, or registrant slots. 50 slots are equivalent of 1 Life day. You can calculate the balance here:</p>
	<input id="slots" type="range" name="slots" min="3000" max="31000"><br>
	<span style="float:left; color: #F00">More Life uptime</span>
	<span style="float:right; color: #08F">More slots</span><br>
	<span style="float:left;" id="uptime"><span id="up">2 days</span> extra Life linked days after 1st of July</span>
	<span style="float:right;" id="extraslots"><span id="slot">5</span> extra slots</span><br><br>
	<center><h1>With this config,<br>Life will unlink <span id="unlinkdate">A</span> with <span id="slotsss">B</span> slots</h1></center>
</div></body>
</html>
